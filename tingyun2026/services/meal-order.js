...
function activeBatches(batches = []) {
  return batches.filter((batch) => !['cancelled', 'canceled'].includes(batch.order_status));
}
...
function totalAmount(batches = []) {
  return activeBatches(batches).reduce((sum, batch) => sum + (Number(batch.regular_amount ?? batch.amount) || 0), 0);
}

function memberTotalAmount(batches = []) {
  return activeBatches(batches).reduce((sum, batch) => sum + (Number(batch.member_amount ?? batch.regular_amount ?? batch.amount) || 0), 0);
}
...
function normalizeOrder(order) {
  ...
  const regularAmount = totalAmount(batches);
  const memberAmount = memberTotalAmount(batches);
...
async function localCreateMealOrder(input = {}) {
  ...
  const orderNo = primary ? primary.order_no : createBusinessId('TYD');
  const batches = primary ? (primary.batches || []) : [];
  const batchNo = batches.length + 1;
  const regularAmount = currentCart.regular_total_amount ?? currentCart.total_amount;
  const memberAmount = currentCart.member_total_amount ?? regularAmount;
  const batch = {
    batch_no: batchNo,
  ...
  if (primary) {
    ...
    primary.total_amount = totalAmount(primary.batches);
    primary.regular_total_amount = primary.total_amount;
    primary.member_total_amount = memberTotalAmount(primary.batches);
    primary.payment_status = 'unpaid';
...
async function createMealOrder(input = {}) {
  ...
  try {
    const order = await callMealCloud('createMealOrder', {
      ...
      customer_type: input.customer_type || user.customer_type || 'guest',
      member_level: input.member_level || user.member_level || '',
      member_level_no: input.member_level_no || user.member_level_no || '',
...
async function cancelMealOrder(input = {}) {
  ...
  const orders = getOrders();
  const order = orders.find((entry) => (entry.order_no || entry.order_id) === orderNo);
  if (!order) return { order_no: orderNo, payment_status: 'unpaid' };
  order.payment_status = 'unpaid';
  order.payment_method = '';
  order.payment_cancelled_at = new Date().toISOString();
  save(orders);
  return { order_no: orderNo, payment_status: order.payment_status };
}

module.exports = { createMealOrder, checkoutMealOrder, listMealOrders, getMealOrderDetail, deleteMealOrder, cancelMealOrder };
