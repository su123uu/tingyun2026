...
async function cancelSignup(input){const order_no=input.order_no||input.signup_id;const items=get();const signup=items.find(x=>(x.order_no||x.signup_id)===order_no);assert(signup,'SIGNUP_NOT_FOUND','未找到活动报名');const activity=find(signup.activity_id);assert(activity.fee_type==='free','PAID_ACTIVITY_CANCEL_NEED_CONTACT','收费活动请联系客服取消');assert(signup.signup_status!=='cancelled','SIGNUP_ALREADY_CANCELLED','该报名已取消');signup.signup_status='cancelled';signup.cancelled_at=new Date().toISOString();save(items);return signup;}
async function listSignups(){return get().map(signup=>{const activity=activities.find(item=>item.activity_id===signup.activity_id);return activity?Object.assign({title:activity.title,image_url:activity.image_url,date:activity.date,time:activity.time,location:activity.location,fee_type:activity.fee_type,can_cancel:activity.fee_type==='free'&&signup.signup_status!=='cancelled'},signup):signup;});}
module.exports={listActivities,getActivityDetail,createSignup,simulateWechatPay,cancelSignup,listSignups};
