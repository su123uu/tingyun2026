...
async function createDiningReservation(input) {
  ...
  const order = Object.assign({
    ...
    room_name: selected.map((room) => room.name).join('、'),
    room_snapshots: selected.map((room) => ({
    ...
    meal_standard_snapshot: {
      ...
      name: standard.name || '',
      price_per_person: standard.price_per_person || 0,
      image: standard.image || standard.image_url || '',
      image_url: standard.image_url || standard.image || '',
    },
    display_items: [{
      id: selectedIds[0] || orderNo,
      image: (selected[0] && (selected[0].image || selected[0].image_url)) || standard.image || standard.image_url || '',
      name: selected.map((room) => room.name).join('、'),
      meta: `${standard.name || ''} · ${input.people_count} 位`,
    }],
...
async function createAccommodationReservation(input) {
  ...
  const order = Object.assign({
    ...
    display_items: selected.map((room) => ({
      id: room.room_id,
      image: room.image || room.image_url || '',
      name: room.name,
      meta: room.category || '',
