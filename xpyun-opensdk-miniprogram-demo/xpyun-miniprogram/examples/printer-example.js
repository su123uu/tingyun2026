...
const formatter = require('../printerlib/util/note-formatter.js');
...
async function printFontAlign() {
     ...
     */
    let printContent = `不加标签：默认字体大小<BR>
<BR>
L标签：<L>左对齐<BR></L>
<BR>
R标签：<R>右对齐<BR></R>
...
BOLD标签：<BOLD>字体加粗<BR></BOLD>`
    ...
    let result = await service.xpYunPrintVoiceSupport(request);
    //正确返回订单编号    
    console.log(JSON.stringify(result));
}
...
async function printComplexReceipt() {
    ...
    printContent = printContent + "菜名" + util.strRepeat(" ", 16) + "数量" + util.strRepeat(" ", 2) + "单价" + util.strRepeat(" ", 2)
        + "<BR>";
